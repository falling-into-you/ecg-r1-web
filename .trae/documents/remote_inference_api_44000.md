...
### 5.4 可信反向代理（Trusted Proxies）
显卡服务器解析 `CF-Connecting-IP / X-Real-IP / X-Forwarded-For` 这类头部时，必须只在请求确实来自“可信反向代理”时才信任，否则客户端可自行伪造这些头。

服务端通过环境变量控制可信代理网段：
- `TRUSTED_PROXY_CIDRS`：默认 `127.0.0.1/32,::1/128`

如果你的“前端服务器”在另一台机器上，并且它会直连显卡服务器 44000，请把前端服务器的出口 IP/网段加入该列表，例如：
- `TRUSTED_PROXY_CIDRS=127.0.0.1/32,::1/128,<frontend-public-ip>/32`

如何确定需要加入的 IP/网段：
- 同一 VPC/内网：加入前端服务器内网网段（例如 `10.0.0.0/16`）。
- 公网访问：加入前端服务器公网出口 IP（建议固定 EIP/固定 NAT）。
